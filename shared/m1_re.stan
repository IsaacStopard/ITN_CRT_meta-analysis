...
functions{
  vector logit_prob_fun_no_cl(int N, matrix pmat_i, matrix pmat_l, matrix pmat_li,
                              ...
                              vector years, real kappa, vector kappa_l){
                          ...
                          logit_prob = alpha + (pmat_i * alpha_i) + theta_ij + // study-specific treatment effect
...
transformed parameters{
  ...
  // mean predictions
  mu_ij = logit_prob_fun_no_cl(N, pmat_i, pmat_l, pmat_li, alpha, alpha_i, theta_l, theta_li, years, kappa, kappa_l);
...
generated quantities{
  ...
  vector[N_gq] mu = gq == 1? logit_prob_fun_no_cl(N_gq, pmat_i_gq, pmat_l_gq, pmat_li_gq, alpha, alpha_i, theta_l, theta_li, years_gq, kappa, kappa_l) : mu_ij;
  vector[N_gq] mu_pooled = logit_prob_fun_no_cl(N_gq, pmat_i_pooled_gq, pmat_l_gq, pmat_li_pooled_gq, alpha, alpha_i, theta_l, theta_li, years_gq, kappa, kappa_l);

   // odds ratios
  vector[N_gq] o_r = exp((pmat_l_gq * theta_l) + (pmat_li_gq * theta_li) + (pmat_l_gq * kappa_l) .* years_gq);

  vector[N_gq] o_r_pooled = exp((pmat_l_gq * theta_l) + (pmat_l_gq * kappa_l) .* years_gq);
