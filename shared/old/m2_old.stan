...
functions{
  vector logit_prob_fun_no_cl(int N, matrix pmat_i, matrix pmat_l, matrix pmat_li,
                        real alpha, vector alpha_i, vector theta_l, vector theta_li, vector m_prob_s, vector d_prob_s,
                        real gamma, real kappa, vector kappa_l, vector years, real omega, real delta,
                        vector omega_l, vector delta_l){
                          ...
                          kappa .* years + (pmat_l * kappa_l) .* years +
                          gamma .* d_prob_s + omega .* m_prob_s + delta .* d_prob_s .* years +
                          (pmat_l * omega_l) .* m_prob_s + (pmat_l * delta_l) .* d_prob_s;
...
transformed parameters{
  ...
  // mean predictions
  mu_ij = logit_prob_fun_no_cl(N, pmat_i, pmat_l, pmat_li, alpha, alpha_i, theta_l, theta_li, m_prob_s, d_prob_s, gamma, kappa, kappa_l, years, omega, delta, omega_l, delta_l);
...
generated quantities{
  ...
  vector[N_gq] mu = gq == 1? logit_prob_fun_no_cl(N_gq, pmat_i_gq, pmat_l_gq, pmat_li_gq, alpha, alpha_i, theta_l, theta_li, m_prob_s_gq, d_prob_s_gq, gamma, kappa, kappa_l, years_gq, omega, delta, omega_l, delta_l) : mu_ij;
  vector[N_gq] mu_pooled = logit_prob_fun_no_cl(N_gq, pmat_i_pooled_gq, pmat_l_gq, pmat_li_pooled_gq, alpha, alpha_i, theta_l, theta_li, m_prob_s_gq_pooled, d_prob_s_gq_pooled, gamma, kappa, kappa_l, years_gq, omega, delta, omega_l, delta_l);

   // odds ratios
  vector[N_gq] o_r = exp((pmat_l_gq * theta_l) + (pmat_li_gq * theta_li) + (pmat_l_gq * kappa_l) .* years_gq + (pmat_l_gq * omega_l) .* m_prob_s_gq + (pmat_l_gq * delta_l) .* d_prob_s_gq);

  vector[N_gq] o_r_pooled = exp((pmat_l_gq * theta_l) + (pmat_l_gq * kappa_l) .* years_gq + (pmat_l_gq * omega_l) .* m_prob_s_gq_pooled + (pmat_l_gq * delta_l) .* d_prob_s_gq_pooled);
  // random effects
